...
message = tk.Label(window, text="얼굴 인식 출입 관리시스템" ,bg="Blue"  ,fg="white"  ,width=50  ,height=3,font=('times', 30, 'italic bold underline')) 
...
message2 = tk.Label(window, text="" ,fg="red"   ,bg="yellow",activeforeground = "green",width=30  ,height=4  ,font=('times', 15, ' bold ')) 
...
def TakeImages():        
    ...
    if(is_number(Id) and name.isalpha()):
        ...
        harcascadePath = r"C:\Users\ASUS\Desktop\Notebook\face_attendance"
        ...
        while(True):
            ...
            for (x,y,w,h) in faces:
                ...
                cv2.imwrite("./Face_attendance/TrainingImage/ "+name +"."+Id +'.'+ str(sampleNum) + ".jpg", gray[y:y+h,x:x+w])
                #display the frame
        ...
        row = [Id , name]
        with open('./Face_attendance/StudentDetails/StudentDetails.csv','a+') as csvFile:
...
def TrainImages():
    ...
    harcascadePath = "./Face_attendance/haarcascade_frontalface_default.xml"
    detector =cv2.CascadeClassifier(harcascadePath)
    faces,Id = getImagesAndLabels("./Face_attendance/TrainingImage")
    recognizer.train(faces, np.array(Id))
    recognizer.save("./Face_attendance/TrainingImageLabel/Trainner.yml")
...
def TrackImages():
    ...
    recognizer.read("./Face_attendance/TrainingImageLabel/Trainner.yml")
    harcascadePath = "./Face_attendance/haarcascade_frontalface_default.xml"
    faceCascade = cv2.CascadeClassifier(harcascadePath);    
    df=pd.read_csv("./Face_attendance/StudentDetails/StudentDetails.csv")
    cam = cv2.VideoCapture(0)
    ...
    while True:
        ...
        for(x,y,w,h) in faces:
            ...
            if(conf < 50):
                ...
                tt=str(Id)  
            if(conf > 75):
                noOfFile=len(os.listdir("./Face_attendance/ImagesUnknown"))+1
                cv2.imwrite("./Face_attendance/ImagesUnknown/Image"+str(noOfFile) + ".jpg", im[y:y+h,x:x+w])            
    ...
    fileName="./Face_attendance/Attendance/Attendance_"+date+"_"+Hour+"-"+Minute+"-"+Second+".csv"
...
copyWrite.insert("insert", "Developed by Darkhorseproject")
